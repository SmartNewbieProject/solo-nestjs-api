...
export class GenerateProfileEmbeddingsCommand extends CommandRunner {
  ...
  async run(
    ...
    try {
      ...
      if (options?.userId) {
        // 특정 사용자의 프로필 임베딩 생성
        console.log(`사용자 ID ${options.userId}의 프로필 임베딩을 생성합니다.`);
      ...
      console.error('프로필 임베딩 생성 중 오류가 발생했습니다:', error.message);
      process.exit(1);
    }
  }
  ...
  private async buildUserProfile(userId: string): Promise<UserProfile | null> {
    try {
      ...
      console.warn(`사용자 ${userId}의 프로필 정보를 가져오는 중 오류 발생:`, error.message);
      return null;
    }
  }
  ...
  private async generateProfileEmbeddingForUser(userId: string): Promise<void> {
    ...
    // 프로필 임베딩 생성
    await this.profileEmbeddingService.generateProfileEmbedding(userId, userProfile);
    ...
    for (const { userId } of userProfiles) {
      try {
        ...
        if (processed % 10 === 0 || processed === userProfiles.length) {
          console.log(`${processed}/${userProfiles.length} 처리 완료 (${Math.round(processed / userProfiles.length * 100)}%)`);
        }
      } catch (error) {
        console.error(`사용자 ${userId}의 프로필 임베딩 생성 중 오류 발생:`, error.message);
      }
    }
  }
