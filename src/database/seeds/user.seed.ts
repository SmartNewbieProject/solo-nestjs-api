...
export class UserSeeder {
  ...
  async seed(count: number = 100, batchSize: number = 20) {
    ...
    // code로 선호도 타입 매핑
    const preferenceTypesByCode = allPreferenceTypes.reduce<
      Record<string, (typeof allPreferenceTypes)[number]>
    >((acc, type) => {
      acc[type.code] = type;
      return acc;
    }, {});
    ...
    const preferenceOptionsByType = allPreferenceOptions.reduce<
      Record<string, PreferenceOption[]>
    >((acc, option) => {
      // preferenceType의 code로 매핑하도록 변경
      const preferenceType = allPreferenceTypes.find(
        (type) => type.id === option.preferenceTypeId,
      );
      if (preferenceType) {
    ...
    for (const typeCode of Object.values(PREFERENCE_TYPE_CODES)) {
      if (
        !preferenceOptionsByType[typeCode] ||
        preferenceOptionsByType[typeCode].length === 0
      ) {
      ...
      try {
        ...
        for (let j = 0; j < currentBatchSize; j++) {
          ...
          await client.query(
            ...
            [userPreferenceId, userId, distanceMax, new Date(), new Date()],
          );
          ...
          for (const preferenceType of allPreferenceTypes) {
            ...
            const typeOptions =
              preferenceOptionsByType[preferenceType.code] || [];
            ...
            if (typeOptions.length > 0) {
              ...
              if (singleOptionTypeCodes.includes(preferenceType.code as any)) {
                ...
                const optionCount = faker.number.int({
                  min: 1,
                  max: Math.min(3, typeOptions.length),
                });
        ...
        if (progress % 25 === 0 || i + currentBatchSize === count) {
          console.log(
            `진행률: ${progress}% (${i + currentBatchSize}/${count} 사용자 생성 완료)`,
          );
        }
      } catch (error) {
        // 오류 발생 시 롤백
  ...
  async clear() {
    ...
    try {
      ...
      const { rows: preferenceOptionRows } = await client.query(
        'SELECT COUNT(*) FROM user_preference_options',
      );
      const { rows: universityDetailRows } = await client.query(
