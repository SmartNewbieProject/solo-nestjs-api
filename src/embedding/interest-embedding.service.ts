...
export class InterestEmbeddingService {
  ...
  async generateAllInterestEmbeddings(): Promise<void> {
    ...
    this.logger.log(
      `${interestOptions.length}개의 관심사 옵션에 대한 임베딩을 생성합니다.`,
    );
    ...
    for (const option of interestOptions) {
      try {
        ...
        this.logger.log(
          `관심사 '${option.displayName}'의 임베딩이 생성되었습니다.`,
        );
      } catch (error) {
        this.logger.error(
          `관심사 '${option.displayName}'의 임베딩 생성 중 오류 발생: ${error.message}`,
          error.stack,
        );
      }
    }
    ...
    if (points.length > 0) {
      ...
      this.logger.log(
        `${points.length}개의 관심사 임베딩이 Qdrant에 저장되었습니다.`,
      );
    }
  ...
  async generateInterestEmbedding(optionId: string): Promise<void> {
    ...
    this.logger.log(
      `관심사 '${option[0].displayName}'의 임베딩이 생성되었습니다.`,
    );
  }
   ...
   */
  async generateUserInterestEmbedding(
    userId: string,
    ...
    // 임베딩 생성
    const embedding =
      await this.embeddingService.createEmbedding(combinedInterests);
  ...
  async findSimilarInterests(
    ...
    Array<{ optionId: string; displayName: string; similarity: number }>
  > {
    // 사용자 임베딩 가져오기
    const userVector = await this.getUserVector(userId);
    if (!userVector) {
  ...
  private async getUserVector(userId: string): Promise<number[] | null> {
    try {
      ...
      this.logger.error(
        `사용자 벡터 가져오기 중 오류 발생: ${error.message}`,
        error.stack,
      );
      return null;
    }
