...
export class S3Service {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!region || !accessKeyId || !secretAccessKey || !bucketName) {
      throw new Error('AWS 설정이 올바르게 구성되지 않았습니다. 환경 변수를 확인하세요.');
    }
   ...
   */
  async uploadFile(file: multer.File, folder: string = 'uploads'): Promise<{ key: string; url: string }> {
    // 파일 확장자 추출
   ...
   */
  async uploadMultipleFiles(files: multer.File[], folder: string = 'uploads'): Promise<{ key: string; url: string }[]> {
