...
export class CommentService {
  ...
  async createComment(
    ...
    const anonymousName = data.anonymous
  ...
  private async checkDuplicateComment(userId: string, data: CommentUpload) {
    const recentComment = await this.commentRepository.getRecentCommentByUser(
      userId,
      5,
    );
    ...
    if (recentComment && recentComment.content === data.content) {
      throw new BadRequestException(
        '동일한 댓글을 너무 빠르게 작성할 수 없습니다. 잠시 후 다시 시도해주세요.',
      );
    }
  }
  ...
  private processComment(comment: CommentWithRelations): CommentDetails {
    // AI 사용자인 경우 가상 정보 생성
    const author =
      comment.author.id === this.AI_USER_ID
        ...
        : {
            ...
            universityDetails: {
              name:
                comment.author.profile.universityDetail?.universityName || '',
              authentication:
                comment.author.profile.universityDetail?.authentication ||
                false,
              department:
                comment.author.profile.universityDetail?.department || '',
              grade: comment.author.profile.universityDetail?.grade || '',
              studentNumber:
                comment.author.profile.universityDetail?.studentNumber || '',
            },
          };
    ...
    return {
      ...
      updatedAt: weekDateService
        .createDayjs(comment.updatedAt || comment.createdAt)
        .format('YYYY-MM-DD HH:mm:ss'),
      createdAt: weekDateService
        .createDayjs(comment.createdAt)
        .format('YYYY-MM-DD HH:mm:ss'),
    };
  }
