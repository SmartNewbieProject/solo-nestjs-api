...
export class ViewCountAggregator {
  ...
  async aggregate(article: ArticleDetails): Promise<ArticleDetails> {
    const memoryViewCount = await this.articleViewService.getViewWithoutDbCount(article.id);
    return {
      ...article,
  ...
  async aggregateList(articles: ArticleDetails[]): Promise<ArticleDetails[]> {
    ...
    const memoryViewCounts = await Promise.all(articleIds.map(id => this.articleViewService.getViewWithoutDbCount(id)));
